import type { Logo } from "@/components/logos/GenericLogo.tsx";

const brandColor = "var(--color-primary)";

const getBrandColor = () => {
    return brandColor;
}

const getTitle = () => {
    return "Sanity";
}

const SanityLogo: Logo = {
    getBrandColor,
    getTitle,
    renderComponent: () => {
        return (
            <svg className={"svg-logo h-3/5 w-3/5"} role="img"  xmlns="http://www.w3.org/2000/svg" fill="var(--color-background)" viewBox="0 0 192 192" height="192" width="192">
                <g clipPath={"url(#clip0_2_1549)"}>
                    <rect fill={brandColor} height="192" width="192"/>
                    <g clipPath={"url(#clip1_2_1549)"}>
                        <path fill={"var(--color-background)"} d="M180.012 117.855L173.386 106.285L141.338 125.872L176.933 80.3953L182.315 77.2267L180.983 75.2195L183.428 72.0847L172.196 62.7029L167.055 69.277L63.2806 130.202L101.649 83.9022L173.113 44.5934L166.323 31.4228L127.399 52.825L146.566 29.7088L135.585 20L92.447 72.0509L49.6034 95.6294L82.4047 52.1146L102.959 41.3797L96.4312 28.0625L36.5484 59.3426L52.8781 37.6585L41.4713 28.491L7 74.2723L7.53486 74.7008L13.975 87.8601L52.1904 67.89L17.3589 114.089L23.0677 118.678L26.4624 125.263L66.6972 103.128L22.3909 156.6L33.372 166.309L35.5769 163.647L142.462 100.692L106.976 146.045L107.554 146.53L107.5 146.564L114.857 159.408L162.055 130.552L143.881 159.983L156.063 168L185 121.147L180.012 117.855Z"/>
                    </g>
                </g>
                <defs>
                    <clipPath id="clip0_2_1549">
                        <rect fill={"var(--color-background)"} height="192" width="192"/>
                    </clipPath>
                    <clipPath id="clip1_2_1549">
                        <rect transform="translate(7 20)" fill={"var(--color-background)"} height="148" width="178"/>
                    </clipPath>
                </defs>
            </svg>
        )
    }
}

export default SanityLogo;